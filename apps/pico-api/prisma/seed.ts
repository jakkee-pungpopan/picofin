...
async function main() {
  ...
  if (!op) {
    await prisma.operator.create({
      data: { name: 'พิโกไฟแนนซ์ บุรีรัมย์', picoType: 'PICO', province: 'บุรีรัมย์',
  ...
  const borrowerUser = await prisma.user.upsert({ where: { email: 'somchai@picofin.local' }, update: {},
    create: { email: 'somchai@picofin.local', fullName: 'สมชาย ใจดี', passwordHash: pw, role: 'BORROWER',
      borrower: { create: { fullName: 'สมชาย ใจดี', nationalId: '1100700123456', province: 'บุรีรัมย์', phone: '0810000001', address: 'อ.เมือง บุรีรัมย์' } } },
  ...
  await prisma.operator.updateMany({ data: { province: 'บุรีรัมย์', name: 'พิโกไฟแนนซ์ บุรีรัมย์' } });
}
